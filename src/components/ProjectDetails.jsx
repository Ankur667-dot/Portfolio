import React from 'react'
import{motion} from 'motion/react'
import close from "../assets/close.svg"
import arrowUp from "../assets/arrow-up.svg"
export default function ProjectDetails({
  title,
  description,
  subDescription,
  image,
  tags,
  href,
  closeModel,
}) {
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center w-full h-full overflow-hidden backdrop-blur-sm'>
      <motion.div className='relative max-w-2xl border shadow-sm rounded-2xl bg-gradient-to-l from-midnight to-navy border-white/10'
      initial={{opacity: 0, scale:0.5}}
      animate={{opacity:1, scale:1}}>
       <button onClick={closeModel} className='absolute p-2 rounded-sm top-5 right-5 bg-midnight hover:bg-gray-500'>
        <img src={close} alt="close-icon" className='w-6 h-6' />
       </button>
        <img src={image} alt={title} className='w-full rounded-t-2xl' />
        <div className='p-5'>
          <h5 className='mb-2 text-2 font-bold text-white'>{title}</h5>
          <p className='mb-3 font-normal text-netural-400 '>{description}</p>
          {subDescription.map((subDesc, index) => (
            <p className='mb-3 font-normal text-netural-400 '>{subDesc}</p>
          ))}
          <div className='flex items-center justify-between '>
            <div>
              {tags.map((tag) => (
                <img key={tag.id} src={tag.path} alt={tag.name} className='rounded-lg size-10 hover-animation' /> 
              ))}
            </div>
            <a className='inline-flex items-center gap-1 font-medium cursor-pointer hover-animation'> View Project 
              <img src={arrowUp} alt="arrow-up" className='size-4 ' href={href} />
            </a>
          </div>
        </div>
      </motion.div>
    </div>
  )
}
